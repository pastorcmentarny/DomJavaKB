...
def generate__full_meal():
    return chinese_utils.get_random_value_from(config.prefix_food) + chinese_utils.get_random_value_from(config.main_food) + chinese_utils.get_random_value_from(config.type_food) + '和喝' + generate_random_non_alkocholic_drink()